...
function Dropdown(props) {
  return (
    <div className="flex absolute  right-6 top-20 bg-gray-100  flex-col justify-center mt-5 ml-5 text-sm text-black border border-solid border-neutral-200 rounded-2xl max-w-[290px]">
      <div className="flex gap-2.5 px-6 pt-6  pb-3 ">
        ...
        />
        <Link to={"/MainProfile"} className="flex-auto self-start  mt-1">My Profile</Link>
      </div>
      <div className="flex flex-col items-start pt-4 pr-20 pb-1.5 pl-6 w-full ">
        <div className="flex gap-3 whitespace-nowrap">
          ...
          />
          <Link to={"/SettingMain"} className="my-auto">Settings</Link>
        </div>
        <div className="flex gap-2.5 mt-7">
          <img
            ...
            className="shrink-0 w-6 aspect-square"
          />
          <Link to={"/aboutus"} className="self-start ">About Us</Link>
        </div>
      </div>
