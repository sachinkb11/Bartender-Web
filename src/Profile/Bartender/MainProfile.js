...
function MainProfile() {
    ...
    const mainEditProfile = <EditProfile></EditProfile>;
    return(
    <WrapperDash>
  {showModal && mainModal}
    {showModalnotify && mainModalnotify}
    {showEditProfile && mainEditProfile}

      <div className="flex gap-5 justify-between px-5 pb-5 mt-3 mb-8 w-full max-md:flex-wrap max-md:pr-5 border-solid border-b-2 max-md:max-w-full">
        <div className="flex gap-5 my-auto  text-3xl font-semibold tracking-wider text-red-500 leading-[50px]">
          <div className="grow my-auto uppercase ">
            <span className=" text-neutral-700 font-[Bebas-Neue]">Your </span>
            <span className="text-yellow-400 font-[Bebas-Neue]">Profile</span>
          </div>
          <img onClick={() => setShowEditProfile((setShowEditProfile) => !setShowEditProfile)}
            loading="lazy"
            src="/images/editProfile.png"
            className=" h-[30px] mt-2 w-[30px]"
          />
        </div>
        <div className="flex gap-5  justify-between text-lg text-white max-md:flex-wrap">
                    <div className="flex gap-5 justify-between">
          <img onClick={() => setShowModalnotify((setShowModalnotify) => !setShowModalnotify)}
            loading="lazy"
        ...
        <div className='flex shadow-lg items-center rounded-lg px-2 border-black gap-3 '>
          ...
          /></div>
          <button  onClick={() => setShowModal((setShowModal) => !setShowModal)} className='text-5xl items-center relative bottom-4 text-black '>&#751;</button>
        </div>
        </div>
        </div>
      </div>
